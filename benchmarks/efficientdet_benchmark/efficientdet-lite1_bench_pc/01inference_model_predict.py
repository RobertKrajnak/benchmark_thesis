...
# model = tf.saved_model.load("../default_efficientdet_models_and_labels/efficientdet_lite3_saved_model")
# print(list(model.signatures.keys()))
# infer = model.signatures["serving_default"]
# print(infer.structured_outputs)

saved_model_loaded = tf.saved_model.load("../default_efficientdet_models_and_labels/efficientdet_lite1_saved_model", tags=[tag_constants.SERVING])
...
batched_input = np.zeros((batch_size, 384, 384, 3), dtype=np.uint8)
...
def get_infertime(mean_iou_list, run, mean_iou_count, batched_input, elapsed_time, elapse_time_on):
    ...
    if elapse_time_on == True:
        ...
        # labeling = model.predict(batched_input)
        # labeling = infer(tf.constant(batched_input))[model.output_names[0]]
    ...
    for j in range(batch_size):
        for i in range(int(num_detection[0])):
            if detection_scores[0, i] > predict_threshold and elapse_time_on == True:
                ...
                scale_imH, scale_imW = imH / 384, imW / 384
                ...
                for ann in bbox_json['annotations']:
                    if ann['image_id'] == id_image:
                        ...
                        bbox_predict = [round(boxes[1] * scale_imW, 2), round(boxes[0] * scale_imH, 2),
                                        round(boxes[3] * scale_imW, 2), round(boxes[2] * scale_imH, 2)]
...
with open("print_out_modelPredict_efficientdet_batch" + str(batch_size) + "_nrun" + str(N_run) + ".txt", 'w') as f:
    sys.stdout = f
