...
# Windows: edgetpu.dll
interpreter = tflite.Interpreter("../default_resnet_models_and_labels/resnet50_model.tflite",
...
def get_infertime(run, top_acc_temp, batched_input, input_details, output_details, elapsed_time, elapse_time_on,
    ...
    for j in range(batch_size):
        ...
        # find = False
        # file = open("../default_resnet_models_and_labels/imagenet-simple-labels.json")
...
for run in range(N_warmup_run):
    elapsed_time, top_acc_temp = get_infertime(run, top_acc_temp, batched_input, input_details, output_details,
                                               elapsed_time,
...
with open("print_out_loadDelegate_resnet_batch"+str(batch_size)+"_topk"+str(k)+"_nrun"+str(N_run)+".txt", 'w') as f:
    sys.stdout = f
