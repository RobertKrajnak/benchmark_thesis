...
model = tf.keras.models.load_model('../default_mobilenet_models_and_labels/mobilenet_v2_saved_model')
...
def get_infertime(run, top_acc_temp, batched_input, elapsed_time, elapse_time_on,
    ...
    for batch in range(batch_size):
        ...
        #
        # batch_input = tf.constant(batched_input)
    ...
    for j in range(batch_size):
        ...
        find = False
        searchfile = open("../default_mobilenet_models_and_labels/imagenet1000_clsidx_to_labels.txt", "r")
...
with open("print_out_modelPredict_mobilenet_batch"+str(batch_size)+"_topk"+str(k)+"_nrun"+str(N_run)+".txt", 'w') as f:
    sys.stdout = f
